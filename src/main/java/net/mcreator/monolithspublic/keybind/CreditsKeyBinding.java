...
package net.mcreator.monolithspublic.keybind;
...
import net.mcreator.monolithspublic.procedures.OpenCreditsGUIProcedure;
...
public class CreditsKeyBinding extends MonolithspublicModElements.ModElement {
	@OnlyIn(Dist.CLIENT)
	private KeyBinding keys;
	private long lastpress = 0;

	public CreditsKeyBinding(MonolithspublicModElements instance) {
		super(instance, 39);
	...
	public void initElements() {
		keys = new KeyBinding("key.monolithspublic.credits", GLFW.GLFW_KEY_0, "key.categories.misc");
		...
		if (Minecraft.getInstance().currentScreen == null) {
			if (event.getKey() == keys.getKey().getKeyCode()) {
				if (event.getAction() == GLFW.GLFW_PRESS) {
					...
					MonolithspublicMod.PACKET_HANDLER.sendToServer(new KeyBindingPressedMessage(1, dt));
	...
	private static void pressAction(PlayerEntity entity, int type, int pressedms) {
		...
		if (type == 1) {
			...
			OpenCreditsGUIProcedure.executeProcedure(Stream
